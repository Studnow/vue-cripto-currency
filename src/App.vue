        ...
        <div class="flex">
          <div class="max-w-xs">
            ...
            >
              <span
                @click="ticker = coin"
                v-for="coin in findTickerInCoins.slice(0, 4)"
                :key="coin"
          ...
          <div
            ...
            :class="{
              'border-4 border-opacity-100':
                selectedTicker == currentTicker.name,
            }"
            class="bg-white border-4 border-opacity-0 overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer"
          >
            ...
            <button
              @click.stop="removeTicker(currentTicker)"
...
export default {
  ...
  watch: {
    ...
    pageStateOptions(stateValue) {
      window.history.pushState(
        null,
        document.title,
        `${window.location.pathname}?filter=${stateValue.filter}&page=${stateValue.page}`
      );
    },
  },
